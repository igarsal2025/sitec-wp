...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => rayforce_theme()->getPluginsManager()->getPluginState( rayforce_get_builder_plugin_slug() ),
			'install_url'                     => rayforce_theme()->getPluginsManager()->getInstallLink( rayforce_get_builder_plugin_slug() ),
			'activate_url'                    => rayforce_theme()->getPluginsManager()->getActivationLink( rayforce_get_builder_plugin_slug() ),
			'slug'                            => rayforce_get_builder_plugin_slug(),
...
<div class="rayforce-admin-big-notice--container">
	<div class="logo-holder">
        <h2><?php echo sprintf(__("Choose one of the %s predefined designs to start with:", 'rayforce'), ucfirst(get_stylesheet())); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
            ...
            $front_page_designs =  array(
                array(
                    'name'    => __( 'Modern', 'rayforce' ),
                    'index'   => 1,
                    'preview' => 'front-page-1.jpg',
                ),

                array(
                    'name'    => __( 'Fullscreen', 'rayforce' ),
                    'index'   => 2,
                    'preview' => 'front-page-2.jpg',
                ),

                array(
                    'name'    => __( 'Classic', 'rayforce' ),
                    'index'   => 3,
            ...
            foreach ( $front_page_designs as $rayforce_design_index => $design ) : ?>
				<?php
					$rayforce_design_selected = $rayforce_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $rayforce_design_selected; ?>">
                    ...
                    <div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( rayforce_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
