...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".rayforce-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.rayforce_admin) {
    return;
  }

  var _window$rayforce_admi = window.rayforce_admin,
      builderStatusData = _window$rayforce_admi.builderStatusData,
      getStartedData = _window$rayforce_admi.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.rayforce-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".rayforce-admin-big-notice").addClass("processing");
      $(".rayforce-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".rayforce-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.rayforce-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".rayforce-admin-big-notice");
  $custom_close_button = $notice_root.find(".rayforce-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".rayforce-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.rayforce_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
